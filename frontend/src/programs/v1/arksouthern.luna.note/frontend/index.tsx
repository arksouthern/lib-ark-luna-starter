...
export default function Notepad(props: App) {
  ...
  const [AboutNotepad, setAboutNotepad] = createAbout({ app: props.app, offsetX: 4, offsetY: 2, sizeX: 30, sizeY: 18 })
  return (
    <A.NotepadExe>
      <XpWindow {...props} buttons={<XpTitleButtonsNormal {...props} />} title={
        ...
        <AboutNotepad icon={"/src/assets/notepad-icon.png"} title={
          <>
            <A.TitleText class="flex-1 pointer-events-none pr-1 tracking-[.032rem] overflow-hidden whitespace-nowrap text-ellipsis">
              About Notepad
            </A.TitleText>
            <XpTitleButtons>
              <XpTitleButtonsClose {...props} onClick={() => setAboutNotepad.dialogHide()} />
            </XpTitleButtons>
          </>
        }>
        ...
        <A.AltBar class="h-5 relative border-b border-white bg-[#ece9d8]">
          <XpBarMenu >
            <XpBarMenuItem name="File">
              <XpBarMenuLineItem shortcut={"Ctrl+N"} disabled={true}>New</XpBarMenuLineItem>
              <XpBarMenuLineItem shortcut={"Ctrl+O"} onClick={async () => {
                const [openPath] = await openFilePicker({})
                self.fileData = (await API.notepadOpenFile({ path: openPath })).string
              }}>Open...</XpBarMenuLineItem>
              <XpBarMenuLineItem shortcut={"Ctrl+S"} disabled={true}>Save</XpBarMenuLineItem>
            ...
            <XpBarMenuItem name="Edit">
              ...
              <XpBarMenuLineItem shortcut={"Ctrl+C"} onClick={async () => {
                self.editor.focus()
                await navigator.clipboard.writeText(self.editor.getModel()?.getValueInRange(self.editor.getSelection() as VSC.IRange) as string)
              }}>Copy</XpBarMenuLineItem>
              <XpBarMenuLineItem shortcut={"Ctrl+V"} onClick={async () => {
                self.editor.trigger("editor", "type", { text: await navigator.clipboard.readText() })
              }}>Paste</XpBarMenuLineItem>
              ...
              <XpBarMenuDivider />
              <XpBarMenuLineItem shortcut={"Ctrl+A"} onClick={() => {
            ...
            <XpBarMenuItem name="Help">
              ...
              <XpBarMenuDivider />
              <XpBarMenuLineItem onClick={() => setAboutNotepad.dialogShow()} >About Notepad</XpBarMenuLineItem>
            </XpBarMenuItem>
          </XpBarMenu>
        </A.AltBar>
        {/* <textarea
