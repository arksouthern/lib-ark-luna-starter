import * as Menu from "@kobalte/core/context-menu"
...
export function XpRightClickMenu(props: { children: any, xpRightClickMenuItemList: any }) {
  return (
    <Menu.ContextMenu>
      <Menu.Trigger>
        {props.children}
      </Menu.Trigger>
      <Menu.Content class="outline-none z-10 grid grid-cols-[1rem_1fr_auto_0.5rem] fade-in animate-in bg-white p-0.5 border border-[#ABA89B] [box-shadow:#646464_4px_4px_3px_-2px]">
        {props.xpRightClickMenuItemList}
      </Menu.Content>
    </Menu.ContextMenu>
  )
}
...
export function XpRightClickMenuItem(props: { onClick?: () => any, disabled?: boolean, icon?: any, indicator?: "checked" | "radio", title: any, shortcut?: any, xpRightClickMenuItemList?: any }) {
  return (
    <>
      {props.xpRightClickMenuItemList ?
        <Menu.Sub overlap gutter={4} shift={-3}>
          <Menu.SubTrigger 
            ...
            class="outline-none group aria-disabled:text-[#ABA89B] grid col-span-4 grid-cols-subgrid pb-0.5 hover:bg-[#4069BF] hover:text-white"
          >
          ...
          <Menu.Portal>
            <Menu.SubContent class="outline-none z-[99999] text-xs -ml-1 grid grid-cols-[1rem_1fr_auto_0.5rem] fade-in animate-in bg-white p-0.5 border border-[#ABA89B] [box-shadow:#646464_4px_4px_3px_-2px]">
              {props.xpRightClickMenuItemList}
            </Menu.SubContent>
          </Menu.Portal>
        </Menu.Sub>
...
export function XpRightClickMenuDivider(props: {}) {
  return (
